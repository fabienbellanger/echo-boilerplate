...
RUN go build -a -installsuffix cgo -o echo-boilerplate .

# Move to /dist directory as the place for resulting binary folder
WORKDIR /dist

# Copy binary from build to main folder
RUN cp /build/echo-boilerplate /build/projects.json /build/favicon.png .
...
COPY --from=builder /dist/echo-boilerplate /
